...
TNode* createNode(int key, Color color, TNode* parent, Tree* RBT) {
    return new TNode{key, nullptr, color, RBT->nil, RBT->nil, parent};
}
...
void leftRotate(Tree* RBT, TNode* x) {
    ...
    y->left = x; // x дочерний левый узел y
...
void insertHuffman(Tree* RBT, int frequency, huffmanNode* huffNode) {
    ...
    TNode* parent = RBT->nil;
    TNode* current = RBT->root;
    
    while (current != RBT->nil) {
        parent = current;
        if (frequency < current->key) {
            current = current->left;
        }
        else if (frequency > current->key) {
